...
class Animate {
    ...
    /**
     Convenience class function for `UIView` animations.
     Asynchronous and includes with default parameter values.
     Defaults can be changed by including them as parameters.
     Chainable via callback function.
     */
    class func custom(animation: Completion? = nil,
                      duration: Double = 0.5,
                      delay: Double = 0,
                      damping: CGFloat = 1,
                      velocity: CGFloat = 1,
    ...
    /**
     ...
     Chainable via callback function.
     */
    class func custom(animation: Completion? = nil, _ duration: Double = 0.5) {
        DispatchQueue.global(qos: .background).async {
            DispatchQueue.main.async {
                UIView.animate(withDuration: duration,
                               delay: 0,
